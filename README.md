...
Stängd app för basketdomare: ladda upp ett klipp direkt från mobilen eller datorn, skriv vad du vill att gruppen tittar på, och diskutera i kommentarer med tidpunkt – klicka på tidpunkten så hoppar spelaren dit och pausar. Domarna skapar konto själva med e-post och lösenord – inget mejlutskick behövs. Nya konton hamnar i en kö och kommer in först när admin godkänner dem; en valfri **registreringskod** kan sättas som filter före kön. Engångskod på mejl finns kvar som alternativ när Resend är uppsatt.
...
**Visselpipan** – ett "jag håller med" på domarspråk – är byggd men avstängd tills vidare. Sätt `VISSELPIPOR_PA` till `true` högst upp i `src/views.ts` för att visa knappen igen; tabellen `whistles` finns redan i schemat.
